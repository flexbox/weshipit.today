import * as React from 'react';
import Link from 'next/link';
import { useRouter } from 'next/router';
import {
  Container,
  MobileContainer,
  InnerGrid,
  MenuButton,
  CloseButton,
  Label,
} from './style';

const NavLinks = ({ activeRoute }: { activeRoute: string }) => {
  return (
    <>
      <Label $isActive={activeRoute === 'Bootcamp'}>
        <Link href="/bootcamp">Bootcamp</Link>
      </Label>

      <Label
        $isActive={
          activeRoute === 'Articles' || activeRoute === 'Mobile Design'
        }
      >
        <Link href="/blog">Articles</Link>
      </Label>

      <Label $isActive={activeRoute === 'About'}>
        <Link href="/about">Why me</Link>
      </Label>

      <Label $isActive={activeRoute === 'Talks'}>
        <Link href="/talks">Talks</Link>
      </Label>

      <Label
        $isActive={
          activeRoute === 'Onboarding' || activeRoute === 'Consultation'
        }
      >
        <Link href="/onboarding">Schedule a call</Link>
      </Label>
    </>
  );
};

export default function Header() {
  const [isExpanded, setExpanded] = React.useState(false);
  const router = useRouter();

  let activeRoute = '';
  let activePath = '/';
  if (router.pathname.includes('/products')) {
    activeRoute = 'Products';
    activePath = '/products';
  }
  if (router.pathname.includes('/about')) {
    activeRoute = 'About';
    activePath = '/about';
  }
  if (router.pathname.includes('/talks')) {
    activeRoute = 'Talks';
    activePath = '/talks';
  }
  if (router.pathname.includes('/mobile')) {
    activeRoute = 'Mobile Design';
    activePath = '/mobile';
  }
  if (router.pathname.includes('/blog')) {
    activeRoute = 'Articles';
    activePath = '/blog';
  }
  if (router.pathname.includes('/onboarding')) {
    activeRoute = 'Onboarding';
    activePath = '/onboarding';
  }
  if (router.pathname.includes('/consultation')) {
    activeRoute = 'Consultation';
    activePath = '/consultation';
  }
  if (router.pathname.includes('/bootcamp')) {
    activeRoute = 'Bootcamp';
    activePath = '/bootcamp';
  }
  if (router.pathname.includes('/onboarding-bootcamp')) {
    activeRoute = 'Bootcamp';
    activePath = '/bootcamp';
  }

  return (
    <>
      <MobileContainer $expanded={isExpanded} data-cy="header">
        {isExpanded ? (
          <>
            <CloseButton
              onClick={() => setExpanded(false)}
              visible={isExpanded}
            >
              ×
            </CloseButton>
            <NavLinks activeRoute={activeRoute} />
          </>
        ) : (
          <>
            <MenuButton onClick={() => setExpanded(true)}>
              <svg
                xmlns="http://www.w3.org/2000/svg"
                fill="currentColor"
                viewBox="0 0 16 16"
                width="16"
                height="16"
              >
                {' '}
                <path
                  fillRule="evenodd"
                  clipRule="evenodd"
                  d="M1 2.75C1 2.55109 1.07902 2.36032 1.21967 2.21967C1.36032 2.07902 1.55109 2 1.75 2H14.25C14.4489 2 14.6397 2.07902 14.7803 2.21967C14.921 2.36032 15 2.55109 15 2.75C15 2.94891 14.921 3.13968 14.7803 3.28033C14.6397 3.42098 14.4489 3.5 14.25 3.5H1.75C1.55109 3.5 1.36032 3.42098 1.21967 3.28033C1.07902 3.13968 1 2.94891 1 2.75ZM1 7.75C1 7.55109 1.07902 7.36032 1.21967 7.21967C1.36032 7.07902 1.55109 7 1.75 7H14.25C14.4489 7 14.6397 7.07902 14.7803 7.21967C14.921 7.36032 15 7.55109 15 7.75C15 7.94891 14.921 8.13968 14.7803 8.28033C14.6397 8.42098 14.4489 8.5 14.25 8.5H1.75C1.55109 8.5 1.36032 8.42098 1.21967 8.28033C1.07902 8.13968 1 7.94891 1 7.75ZM1.75 12C1.55109 12 1.36032 12.079 1.21967 12.2197C1.07902 12.3603 1 12.5511 1 12.75C1 12.9489 1.07902 13.1397 1.21967 13.2803C1.36032 13.421 1.55109 13.5 1.75 13.5H14.25C14.4489 13.5 14.6397 13.421 14.7803 13.2803C14.921 13.1397 15 12.9489 15 12.75C15 12.5511 14.921 12.3603 14.7803 12.2197C14.6397 12.079 14.4489 12 14.25 12H1.75Z"
                ></path>
              </svg>
            </MenuButton>
            <Link href={activePath}>{activeRoute}</Link>
          </>
        )}
      </MobileContainer>

      <Container data-cy="header">
        <InnerGrid>
          <NavLinks activeRoute={activeRoute} />
        </InnerGrid>
      </Container>
    </>
  );
}
